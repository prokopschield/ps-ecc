...
impl ReedSolomon {
    ...
    ///   `parity` holds more than [`MAX_PARITY_BYTES`] bytes, or an odd
    ///   number of bytes.
    ...
    pub fn correct_detached_data_in_place(
        ...
        // the u8 conversion below; the length bound also keeps `parity`
        // within the stack-allocated validation buffer used further down.
